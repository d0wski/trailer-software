...
export async function addBooking(booking) {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      ice_bag_qty: booking.iceBagQty || 0,
      ice_price_per_bag: booking.icePricePerBag || 0,
      round_trip_miles: booking.roundTripMiles || 0,
      price_per_mile: booking.pricePerMile || 1.25
    })
    .select()
    .single();
...
export async function updateBooking(id, updates) {
  ...
  if (updates.iceBagQty !== undefined) dbUpdates.ice_bag_qty = updates.iceBagQty;
  if (updates.icePricePerBag !== undefined) dbUpdates.ice_price_per_bag = updates.icePricePerBag;
  if (updates.roundTripMiles !== undefined) dbUpdates.round_trip_miles = updates.roundTripMiles;
  if (updates.pricePerMile !== undefined) dbUpdates.price_per_mile = updates.pricePerMile;
