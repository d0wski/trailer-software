...
import { getTrailers, addTrailer, updateTrailer, deleteTrailer, getBookingsForTrailer } from '../store.js';
...
export async function loadEquipmentView() {
  ...
  // Fetch bookings for each trailer to determine rental status
  const trailerStatuses = await Promise.all(
    trailers.map(async (t) => {
      const bookings = await getBookingsForTrailer(t.id);
